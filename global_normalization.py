...
# ------------ paths ---------------------------------------------------------
INPUT_PATH  = "signal_features2_std_gender_no_mfcc_norm.csv"
...
if filename_col:
    ...
    print(f"🗣️   Unique speakers detected (info only): {df['speaker'].nunique(dropna=True)}")
else:
    print("⚠️  No filename column found; skipping speaker extraction.")
...
print(f"🗣️   Final unique speakers in output (for reference): {normalized_df['speaker'].nunique(dropna=True) if 'speaker' in normalized_df else 'N/A'}")
